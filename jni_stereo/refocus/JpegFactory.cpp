...
bool JpegFactory::jpgDecode(char const *fileName, uint8_t *dstBuffer, uint32_t dstWidth, uint32_t dstHeight) {
    FILE *fp = NULL;
    ...
    ALOGI("<jpgDecode>open file %s success!", fileName);
    // get file size
    fseek(fp, SEEK_SET, SEEK_END);
    file_size = ftell(fp);
    ALOGI("<jpgDecode>file size is %d", file_size);
    ...
    ALOGI("<jpgDecode>read file to buffer,srcSize: %d", file_size);
    ...
    timeDiff = getTimeDiff(startSec, startUsec, endSec, endUsec);
    ALOGI("<jpgDecode>%d ==> jpgToYV12: jpg to yv12", timeDiff);
    free(file_buffer);
    fclose(fp);
    ALOGI("<jpgDecode>end");
    return true;
...
bool JpegFactory::jpgToYV12(uint8_t* srcBuffer, uint32_t srcSize, uint8_t *dstBuffer, uint32_t dstWidth,
        uint32_t dstHeight) {
    MHAL_JPEG_DEC_INFO_OUT outInfo;
    MHAL_JPEG_DEC_START_IN inParams;
    MHAL_JPEG_DEC_SRC_IN    srcInfo;
    void *fSkJpegDecHandle = NULL;
...
void JpegFactory::getTime(long *sec, long *usec) {
    timeval time;
...
long JpegFactory::getTimeDiff(long startSec, long startNSec, long endSec, long endNSec) {
