...
class FancyColorJni {
    ...
    public Bitmap getFancyColorEffectImage(Bitmap src, String effectName, int type) {
        ...
        if(effectName.equals(FancyColorHelper.EFFECT_NAME_MONO_CHROME)){
        	 return imageFilterBwFilter(temp, mask);
        }
        else if( effectName.equals(FancyColorHelper.EFFECT_NAME_POSTERIZE)){
        	return imageFilterKMeans(temp, mask);
        }
        else{
            int effectIndex = mAllEffectArray.indexOf(effectName);
            return (Bitmap) getFancyColorEffectImage(effectIndex, temp, mask, src.getWidth(),
                    src.getHeight(), rect, point.x, point.y);
        }
    }
    ...
    public boolean reloadMaskBuffer(Point point) {
        ...
        return getPreviewMask(mOriPreviewBitmap);
    }
