...
public class StereoRefineActivity extends Activity {
    ...
    private String saveRefineResult() {
        ...
        String filename = new SimpleDateFormat("_yyyyMMdd_HHmmss_SSS").format(new Date(System
                .currentTimeMillis()));
        final File target = new File(folder, "clip" + filename + ".jpg");
        ...
        new Thread(new Runnable() {
            public void run() {
                ...
                try {
                    ...
                    toSaveBitmap.compress(CompressFormat.PNG, SAVE_QUALITY, fos);
                    fos.close();
                } catch (FileNotFoundException e) {
                    MtkLog.i(TAG, "<saveRefineResult> FileNotFoundException");
                } catch (IOException e) {
                    MtkLog.i(TAG, "<saveRefineResult> IOException");
                }
