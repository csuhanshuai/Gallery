...
public class XmpOperator {
    ...
    private ExtendXmpOperator mExtOperator;
    ...
    /**
     * Stereo parameters needed by Google.
     */
    public static class GoogleStereoParams {
        public double mGFocusBlurInfinity;
        public double mGFocusFocalDistance;
        public double mGFocusFocalPointX;
        public double mGFocusFocalPointY;
        public double mGDepthNear;
        public double mGDepthFar;
        public String mGDepthMime;
        public String mGImageMime;
        public String mGDepthFormat;

        /**
         * Constructor.
         */
        public GoogleStereoParams() {
            mGDepthMime = MIME_PNG;
            mGImageMime = MIME_JPEG;
            mGDepthFormat = DEPTH_FORMAT;
            mGFocusBlurInfinity = DEFAULT_VALUE;
            mGFocusFocalDistance = DEFAULT_VALUE;
            mGFocusFocalPointX = DEFAULT_VALUE;
            mGFocusFocalPointY = DEFAULT_VALUE;
            mGDepthNear = DEFAULT_VALUE;
            mGDepthFar = DEFAULT_VALUE;
        }
    }
     ...
     */
    public XmpOperator() {
        mXmpInterface = new XmpInterface(mRegister);
        mSegmentMaskOperator = new SegmentMaskOperator(mXmpInterface);
        mExtOperator = new ExtendXmpOperator();
    }
     ...
     * @return success->true,fail->false
     */
    public byte[] writeStereoCaptureInfoToJpg(String fileName, byte[] jpgBuffer,
            byte[] jpsData, byte[] jsonBuffer, byte[] clearImage, byte[] gDepthMapSmall) {
        ...
        if (clearImage == null && gDepthMapSmall == null) {
        ...
        Log.d(TAG, "<writeStereoCaptureInfoToJpg> jsonBuffer size: " + jsonBuffer.length);
        ...
        byte[] encodedDepth = null;
        if (gDepthMapSmall != null) {
            encodedDepth = Utils.encodePng(gDepthMapSmall, parser.getGoogleDepthWidth(),
        ...
        try {
            ...
            os = new ByteArrayOutputStreamExt();
            writeStereoCaptureInfo(is, os, jpsData, jsonBuffer, jpsMask, clearImage, encodedDepth);
            byte[] out = os.toByteArray();
            if (ENABLE_BUFFER_DUMP && fileName != null) {
                XmpInterface.writeBufferToFile(
                        DUMP_PATH + DUMP_FOLDER_NAME + mFileName + "Out.jpg", out);
    ...
    public byte[] getClearImage(String filePath) {
        if (mExtOperator == null || filePath == null) {
        ...
        if (ENABLE_BUFFER_DUMP) {
            XmpInterface.writeBufferToFile(DUMP_PATH + DUMP_FOLDER_NAME + mFileName
                    + "ExtendXMP_RealData_Read.xml", mExtendXmpData);
        }
        mExtOperator.setSrcBuffer(mExtendXmpData);
        byte[] clearImageBuffer = mExtOperator.getImageBuffer();
        if (ENABLE_BUFFER_DUMP) {
            XmpInterface.writeBufferToFile(DUMP_PATH + DUMP_FOLDER_NAME + mFileName
    ...
    public byte[] getGoogleDepthMap(String filePath) {
        ...
        mExtOperator.setSrcBuffer(mExtendXmpData);
        byte[] googleDepthMap = Utils.decodePng(mExtOperator.getGoogleDepthMap());
        Log.d(TAG, "<getGoogleDepthMap> end!!!");
        return googleDepthMap;
    }

    /**
     * Replace clear image in src file with blurImage.
     *
     * @param srcFilePath
     *            src file
     ...
     */
    public boolean replaceBlurImage(String srcFilePath, String blurImageFilePath) {
        Log.d(TAG, "<replaceBlurImage> write begin!!!");
        String tempPath = srcFilePath + ".tmp";
        // delete source file and rename new file to source file
        boolean status = replaceBlurImage(srcFilePath, blurImageFilePath, tempPath);
    ...
    private boolean replaceBlurImage(String srcFilePath, String blurImageFilePath,
            String outputPath) {
        ...
        try {
            ...
            // copy src file from 0 ~ DQT to output file
            Section sectionDQT = getDQTSection(srcFileSections);
            if (sectionDQT == null) {
                Log.d(TAG, "<replaceBlurImage> parse src DQT section error!!");
                return false;
            }
            mXmpInterface.copyFileWithFixBuffer(srcFileRafIn, rafOut, 0, sectionDQT.mOffset);

            // copy blur image file from DQT ~ end to output file
            ...
            for (int i = 0; i < mParsedSectionsForGallery.size(); i++) {
                ...
                if (sec.mIsXmpExt) {
                    rafIn.seek(sec.mOffset + 2 + 2
                            + ExtendXmpOperator.EXT_XMP_COMMON_HEADER_TOTAL_LEN_OFFSET);
            ...
            for (int i = 0; i < mParsedSectionsForGallery.size(); i++) {
                ...
                if (sec.mIsXmpExt) {
                    ...
                    extXmpPartitionLen = app1Len
                            - ExtendXmpOperator.EXT_XMP_COMMON_HEADER_REAL_DATA_OFFSET;
                    // seek to real extend xmp data
                    rafIn.skipBytes(ExtendXmpOperator.EXT_XMP_COMMON_HEADER_REAL_DATA_OFFSET);
    ...
    private void writeStereoCaptureInfo(ByteArrayInputStreamExt is, ByteArrayOutputStreamExt os,
        ...
        if (writenLocation == XmpInterface.WRITE_XMP_AFTER_SOI) {
            // means no APP1
            writeConfigToStream(os, jsonBuffer, clearImage, gDepthMapSmall);
    ...
    private void setGoogleStereoParams(XMPMeta meta, GoogleStereoParams stereoParams) {
    ...
    private XMPMeta makeXmpMainDataInternal(XMPMeta meta, StereoDebugInfo configInfo) {
        ...
        setGoogleStereoParams(meta, new GoogleStereoParams());
        mXmpInterface.registerNamespace(MEDIATEK_IMAGE_REFOCUS_NAMESPACE, MTK_REFOCUS_PREFIX);
        ...
        mXmpInterface.setPropertyBoolean(meta, MEDIATEK_IMAGE_REFOCUS_NAMESPACE, XMP_DEPTH_FLAG,
                false);
        makeXmpMainDataforSegment(meta, configInfo);
        updateMd5ValueInXmpMain(meta);
        return meta;
    }
    ...
    private void updateMd5ValueInXmpMain(XMPMeta meta) {
        ...
        mXmpInterface.setPropertyString(meta, NS_XMPNOTE, ATTRIBUTE_XMPNOTE, "");
        if (mExtOperator == null) {
            Log.d(TAG, "<updateMd5ValueInXmpMain> mExtOperator is null, error!!");
            return;
        }
        String md5 = mExtOperator.getMd5Value();
        Log.d(TAG, "<updateMd5ValueInXmpMain> md5 " + md5);
        mXmpInterface.setPropertyString(meta, NS_XMPNOTE, ATTRIBUTE_XMPNOTE, md5);
    }

    private void makeXmpMainDataforSegment(XMPMeta meta, StereoDebugInfo configInfo) {
        // add for segment
    ...
    private void writeConfigToStream(ByteArrayOutputStreamExt os, byte[] jpsConfig,
            byte[] clearImage, byte[] gDepthMapSmall) {
        try {
            ...
            ArrayList<byte[]> extXmp = makeXmpExtData(clearImage, gDepthMapSmall);
    ...
    private ArrayList<byte[]> makeXmpExtData(byte[] clearImage, byte[] gDepthMapSmall) {
        if (mExtOperator == null || (clearImage == null && gDepthMapSmall == null)) {
            Log.d(TAG, "<makeXmpExtData> mExtOperator is null or jade implement!!");
            return new ArrayList<byte[]>();
        }
        mExtOperator.debugConfig(ENABLE_BUFFER_DUMP, DUMP_PATH + DUMP_FOLDER_NAME + mFileName);
        return mExtOperator.makeXmpExtData(clearImage, gDepthMapSmall);
    }
    ...
    private boolean updateOnlyDepthInfo(RandomAccessFile rafOut, XMPMeta meta,
        ...
        mExtOperator.debugConfig(ENABLE_BUFFER_DUMP, DUMP_PATH + DUMP_FOLDER_NAME + mFileName);
        ArrayList<byte[]> extXmpDataArray = mExtOperator.updateGoogleDepthMap(mExtendXmpData,
    ...
    private boolean writeOnlyDepthBuffer(RandomAccessFile rafOut, DepthBufferInfo depthBufferInfo) {
        if (rafOut == null || depthBufferInfo == null || mExtOperator == null) {
